...
class ReceiptSettings extends Table {
  ...
  TextColumn get branchId => text().unique().references(Branches, #id)();
  ...
  TextColumn get logoPosition => text().withDefault(const Constant('top'))();
  ...
  /// Print "Pelanggan: Nama (phone)" in the receipt meta section when a
  /// customer is attached to the transaction/cart.
  BoolColumn get showCustomerName =>
      boolean().withDefault(const Constant(true))();

  /// Print the branch display name at the top of the receipt.
  ...
  TextColumn get locale => text().withDefault(const Constant('id_ID'))();
